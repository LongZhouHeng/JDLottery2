...
public class NewKsOpenAPI extends BasicRequest {
    ...
    private String uid;
    public NewKsOpenAPI(Activity activity,String uid, String table_name, int PageNum, String PageSize, RequestListener mListener) {
    ...
    public void onSucceed(int what, Response<String> response) {
        try {
            ...
            model = JSON.parseObject(response.get(), AllKsLotModel.class);
            System.out.println("BBBBBBBBBBBB"+model);
            if (model.msg.equals("success")) {
                mListener.onComplete(new BasicResponse<AllKsLotModel>(model));
            } else {
